...
export const get_admin_dashboard_data = createAsyncThunk(
        ...
        async (_, { fulfillWithValue, rejectWithValue }) => {
                try {
                        const response = await api.get(`/dashboard/admin-dashboard-data`, {
...
const dashboardReducer = createSlice({
        ...
        extraReducers: (builder) => {
                ...
                builder.addCase(get_seller_dashboard_data.pending, (state) => {
                        state.loader = true;
                });
                builder.addCase(get_seller_dashboard_data.fulfilled, (state, action) => {
                        state.loader = false;
                        state.totalSale = action.payload.data.totalSale;
                        state.totalOrder = action.payload.data.totalOrder;
                        state.totalProduct = action.payload.data.totalProduct;
                        state.totalPendingOrder = action.payload.data.totalPendingOrder;
                        state.recentOrders = action.payload.data.recentOrders;
                        state.recentMessages = action.payload.data.messages;
                });
                builder.addCase(get_seller_dashboard_data.rejected, (state, action) => {
                        state.loader = false;
                        state.errorMessage = action.payload;
                });
        },
